...
void TensorIteratorBase::build_inconsistent_unary_op(const Tensor& out, const Tensor& a) {
  build(TensorIteratorConfig()
      .set_check_mem_overlap(true)
      .add_output(out)
      .add_input(a)
...
TensorIterator TensorIterator::casting_unary_op(Tensor& out, const Tensor& a) {
  TensorIterator iter;
  iter.build_inconsistent_unary_op(out, a);
  return iter;
}
