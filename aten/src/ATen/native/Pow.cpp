...
#define CREATE_FLOAT_POWER_OUTPUT(result, t, dtype)                                                                              \
  if (result.defined()) {                                                                                                        \
    TORCH_CHECK(result.scalar_type() == dtype : true,                                                                            \
    ...
    set_output(0, base.sizes(), {}, base.options().dtype(dtype), base.has_names() ? base.names() : ArrayRef<Dimname>());         \
  }
...
TORCH_META_FUNC2(float_power, Tensor_Tensor) (const Tensor& base, const Tensor& exp) {
  ...
  CREATE_FLOAT_POWER_OUTPUT(maybe_get_output(), base, dtype);
}
...
TORCH_META_FUNC2(float_power, Scalar) (const Scalar& base, const Tensor& exp) {
  ...
  CREATE_FLOAT_POWER_OUTPUT(maybe_get_output(), exp, dtype);
}
